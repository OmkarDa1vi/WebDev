...
select * from Restaurant where City = 'New Delhi';
...
update Restaurant set AvgCost = 2500 where RestaurantName = 'TheTable';

-- 6. Delete operation: Remove the record from the Restaurants table where Restaurant_Name is 'The Table'
delete from Restaurant where RestaurantName = 'The Table';
...
select RestaurantName, City from Restaurant where City =  'Mumbai';

-- 11. Displaying specific details: Retrieve Restaurant_Name, City, and Cuisines from Restaurants table where City is 'Mumbai'
select RestaurantName, City, Cuisine from Restaurant where City =  'Mumbai';
...
select * from Restaurant order by AvgCost desc;
